...
class Vocabulary:
    ...
    def build(self, token_stream):
        ...
        filtered_words = [word for word, count in counter.items() if count >= self.min_freq]
        print(f"Keeping {len(filtered_words)} tokens with min frequency {self.min_freq}")
        ...
        self.frequencies = {0: sum(count for word, count in counter.items() if count < self.min_freq)}
