...
class SkipGramModel(nn.Module):
    ...
    def forward(self, centers, contexts, negatives):
        ...
        center_emb = self.in_embed(centers)                     # [batch_size, embed_dim]
        context_emb = self.out_embed(contexts)                  # [batch_size, embed_dim]
        negative_emb = self.out_embed(negatives)                # [batch_size, n_negatives, embed_dim]
        ...
        center_emb = center_emb.unsqueeze(2)                    # [batch_size, embed_dim, 1]
        
        # Compute negative scores (batch matrix multiplication)
        neg_score = torch.bmm(negative_emb, center_emb).squeeze(2)  # [batch_size, n_negatives]
