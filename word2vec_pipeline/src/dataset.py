...
class SkipgramDataset(IterableDataset):
    ...
    def _get_negative_samples(self, positive_idx, n_samples):
        ...
        """
        indices = np.random.randint(0, len(self.neg_sampling_table), size=n_samples + 10)
        ...
        if len(samples) < n_samples:
            more_samples = self._get_negative_samples(positive_idx, n_samples - len(samples))
    ...
    def __iter__(self):
        ...
        for tokens in self.token_stream:
            for center, context in generate_skipgram_pairs(tokens, self.vocab, self.window_size):
                neg_samples = self._get_negative_samples(context, self.neg_samples)
