...
class TitleVectorizer:
    ...
    def __init__(self, word_vectors_path=None, model_path=None):
        ...
        if word_vectors_path is None:
            word_vectors_path = str(
                base_dir / 'word2vec_pipeline/embeddings/word_vectors_final.npy')

        if model_path is None:
            model_path = str(
                base_dir / 'word2vec_pipeline/models/best_skipgram_model.pth')
    ...
    def get_title_vector(self, title):
        ...
        try:
            ...
            print(
                f"Warning: Non-string title encountered: {title} (type: {type(title)})")
            return np.zeros(self.embed_dim)
        ...
        for token in tokens:
            ...
            # Avoid index 0 which is usually <UNK>
            idx = (hash_value % (vocab_size - 1)) + 1
        ...
        if precompute and len(titles) <= max_precompute:
            ...
            for i, title in enumerate(titles):
                self.title_vectors.append(
                    self.vectorizer.get_title_vector(title))
...
def train_and_save_model(model_save_path="model.pth", force_retrain=False, two_stage=True):
    ...
    for i in range(5):  # Just show first 5 samples
        print(
            f"  - \"{train_data[i]['title']}\" (Score: {train_data[i]['score']})")
    ...
    print(
        f"After filtering: {len(all_titles)}/{len(raw_titles)} valid samples remaining")
    ...
    def train_model(titles, scores, model_path, epochs=20, lr=0.002,
        ...
        for epoch in range(epochs):
            ...
            for batch in dataloader:
                ...
                if batch_count % 1000 == 0:
                    print(
                        f"  Batch {batch_count}/{len(dataloader)}, Current loss: {loss.item():.4f}")
            ...
            if epoch_loss < best_loss:
                ...
                try:
                    ...
                    print(
                        f"  ✓ New best model saved to {best_path} with loss: {best_loss:.4f}")
                ...
                no_improve_count += 1
                print(
                    f"  - No improvement for {no_improve_count} epochs. Best loss: {best_loss:.4f} at epoch {best_epoch}")

            # Early stopping check
            if no_improve_count >= patience:
                print(
                    f"Early stopping at epoch {epoch+1} as no improvement for {patience} epochs")
                break
        ...
        if best_state is not None:
            ...
            print(
                f"Loaded best model from epoch {best_epoch} with loss {best_loss:.4f}")
    ...
    if two_stage:
        ...
        final_model, _ = train_model(
            ...
            model_save_path, epochs=25, lr=0.001,
        ...
        final_model, _ = train_model(
            all_titles, all_scores,
            model_save_path, epochs=25, lr=0.002,
            batch_size=256, patience=5,
            description="Training on full dataset"
        )
        ...
        if os.path.exists(model_save_path):
            model_size = os.path.getsize(
                model_save_path) / (1024 * 1024)  # Size in MB
            print(
                f"Model successfully saved to {model_save_path} ({model_size:.2f} MB)")
        else:
        ...
        if os.path.exists(best_model_path):
            model_size = os.path.getsize(
                best_model_path) / (1024 * 1024)  # Size in MB
            print(
                f"Best model saved to {best_model_path} ({model_size:.2f} MB)")
